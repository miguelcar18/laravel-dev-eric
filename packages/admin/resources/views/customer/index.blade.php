...
@section('page-title')
    {{ trans('admin::pages/customer.index.page_title') }}
@endsection
...
@section('page-header')
{{ trans('admin::pages/customer.index.page_header') }}
@endsection

@section('breadcrumbs')
@include('admin::partials.page.breadcrumbs', [ 'breadcrumbs' => [
    ['text' => trans('admin::pages/customer.index.breadcrumb')]
]])
@endsection
                    ...
                    <table id="server-side-data-table" class="table table-bordered table-hover table-striped table-light-dark">
                        <thead class="thead-default">
                            <tr>
                                ...
                                <th>{{ trans('admin::pages/customer.index.columns.mobile') }}</th>
    ...
    <script type="text/javascript">
        const tableActionButtons = (data) => {
            ...
            if (data.can_edit) {
                ...
                html+='title="{{ trans('admin::pages/customer.index.edit') }}"';
                html+='><i class="fa fa-edit"></i></button>';
            }

            if (data.can_delete) {
                html+='<button class="btn btn-outline-danger btn--icon"';
                html+='title="{{ trans('admin::pages/customer.index.destroy.tooltip') }}"';
                html+='data-trigger="sweet-alert"';
                html+='data-sweet-alert-title="{{ trans('admin::pages/customer.index.destroy.alert.title') }}"';
                html+='data-sweet-alert-text="{{ trans('admin::pages/customer.index.destroy.alert.text') }}"';
                html+='data-sweet-alert-type="error"';
                html+='data-sweet-alert-confirm-text="{{ trans('admin::pages/customer.index.destroy.alert.confirm') }}"';
                html+='data-sweet-alert-confirm-action="document.getElementById(\'delete-' + data.id + '\').submit()"';
                html+='data-sweet-alert-cancel-text="{{ trans('admin::pages/customer.index.destroy.alert.cancel') }}"';
        ...
        $(document).ready(function() {
            var table = $('#server-side-data-table').DataTable( {
                ...
                ajax: {
                    url: "{{ route('admin::api.customer.index') }}",
                    data: function ( d ) {
                ...
                columns: [
                    { data: 'name', name: 'name', render: function(data, type, row, meta) {
                        if(row.can_edit) {
                            return '<a href="'+row.edit_route+'" title="{{ trans('admin::pages/customer.index.edit') }}">' + data + '</a>';
                        }
                        return data;
                    } },
                ...
                initComplete: function(settings, json) {
                    ...
                    let create_button = {
                        ...
                        title: "{{ trans('admin::pages/customer.create.page_title') }}"
                    };
