...
@section('page-title')
    {{ trans('admin::pages/group.group_permission.page_title') }}
@endsection
...
@section('page-header')
    - {{ trans('admin::pages/group.group_permission.page_header') }}
@endsection
...
@section('breadcrumbs')
    @include('admin::partials.page.breadcrumbs', [ 'breadcrumbs' => [
        ...
        ['text' => trans('admin::pages/group.group_permission.breadcrumb')]
    ]])
@endsection
                ...
                <div class="row">
                    <div class="table-responsive">
                        <table id="server-side-data-table" class="table table-bordered table-hover table-striped table-light-dark">
                            ...
                            <tr>
                                <th>{{ trans('admin::pages/permission.index.columns.name') }}</th>
                                <th>{{ trans('admin::pages/permission.index.columns.actions') }}</th>
                            </tr>
                            </thead>
                        </table>
                    </div>
    ...
    <script type="text/javascript">
        const tableActionButtons = (data) => {
            ...
            // if (data.can_show) {
                ...
                html+='<input class="form-check-input checkStatus" data-id="'+ data.id+ '" id="isChecked" type="checkbox" value="permission_id" >';
                html+='</div>';
        ...
        $(document).ready(function() {
            var table = $('#server-side-data-table').DataTable( {
                ...
                ajax: {
                    url: "{{ route('admin::api.group.group_permission') }}",
                    data: function ( d ) {
                ...
                columns: [
                    { data: 'name', name: 'name', render: function(data, type, row, meta) {
                            if(row.can_edit) {
                                return '<a href="'+row.edit_route+'" title="{{ trans('admin::pages/permission.index.edit') }}">' + data + '</a>';
                            }
                            return data;
                        } },
            ...
            $(document).on('change','select[name=group_id]',function (event){
                ...
                if (!select.is('change')){
                    location.href = "{{ route('admin::group.group_permission', 'A_FAKE_ID_TO_REPLACE') }}".replace('A_FAKE_ID_TO_REPLACE', $("select[name=group_id]").val());
                }
            })
            ...
            $(document).on('change','input.checkStatus',function (event) {
                let checkbox = $(this);
                let idPermission = checkbox.data('id');
                ...
                $.ajax({
                    url:'{{ route('admin::api.group.assign_permission') }}',
                    type: 'post',
                    data: {
