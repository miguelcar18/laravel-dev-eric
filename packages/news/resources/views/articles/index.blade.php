...
@section('page-title')
    {{ trans('news::pages/article.index.page_title') }}
@endsection
...
@section('page-header')
    {{ trans('news::pages/article.index.page_header') }}
@endsection

@section('breadcrumbs')
    @include('admin::partials.page.breadcrumbs', [ 'breadcrumbs' => [
    ['text' => trans('news::pages/article.index.breadcrumb')]
]])
@endsection
            ...
            <div class="row">
                ...
                <div class="table-responsive">
                    <table id="server-side-data-table"
                        ...
                        <tr>
                            <th>{{ trans('news::pages/article.index.columns.title') }}</th>
                            <th>{{ trans('news::pages/article.index.columns.subtitle') }}</th>
                            <th>{{ trans('news::pages/article.index.columns.section') }}</th>
                            <th>{{ trans('news::pages/article.index.columns.author_id') }}</th>
                            <th>{{ trans('news::pages/article.index.columns.actions') }}</th>
                        </tr>
                        </thead>
                    </table>
                </div>
    ...
    <script type="text/javascript">
        const tableActionButtons = (data) => {
            ...
            // if (data.can_edit) {
                ...
                html += 'title="{{ trans('news::pages/article.index.edit') }}"';
            ...
            // if (data.can_show) {
                ...
                html += 'title="{{ trans('news::pages/article.index.show') }}"';
                html += '><i class="fa fa-eye"></i></button> &nbsp;';
            // }

            // if (data.can_delete) {
                html += '<button class="btn btn-outline-danger btn--icon"';
                html += 'title="{{ trans('news::pages/article.index.destroy.tooltip') }}"';
                html += 'data-trigger="sweet-alert"';
                html += 'data-sweet-alert-title="{{ trans('news::pages/article.index.destroy.alert.title') }}"';
                html += 'data-sweet-alert-text="{{ trans('news::pages/article.index.destroy.alert.text') }}"';
                html += 'data-sweet-alert-type="error"';
                html += 'data-sweet-alert-confirm-text="{{ trans('news::pages/article.index.destroy.alert.confirm') }}"';
                html += 'data-sweet-alert-confirm-action="document.getElementById(\'delete-' + data.id + '\').submit()"';
                html += 'data-sweet-alert-cancel-text="{{ trans('news::pages/article.index.destroy.alert.cancel') }}"';
        ...
        $(document).ready(function () {
            var table = $('#server-side-data-table').DataTable({
                ...
                ajax: {
                    url: "{{ route('news::api.article.index') }}",
                    data: function (d) {
                ...
                columns: [
                    {
                        data: 'title', name: 'title', render: function (data, type, row, meta) {
                            if (row.can_edit) {
                                return '<a href="' + row.edit_route + '" title="{{ trans('news::pages/article.index.edit') }}">' + data + '</a>';
                            }
                            return data;
                        }
                    },
                    {data: 'subtitle', name: 'title'},
                ...
                initComplete: function (settings, json) {
                    ...
                    let create_button = {
                        url: "{{ route('news::article.create') }}",
                        title: "{{ trans('news::pages/article.create.page_title') }}"
                    };
